...
void MainProgram::connectPanelDBus()
{
    ...
    connect(m_backupDialog, &BackupDialog::signal_restore_lexicon_and_settings_ok, m_panelDBusService,
            &QDBusPanelService::ReloadConfig);
...
void MainProgram::connectSettingsDBus()
{
    ...
    connect(s, &QDBusSettingsService::signal_switch_input_mode, this, [this](const QString &inputMode) {
    ...
    connect(s, &QDBusSettingsService::signal_dict_query, m_dictQueryWin, &DictQueryWin::slot_open_win);

    connect(s, &QDBusSettingsService::signal_generate_usr_word, this,
    ...
    connect(s, &QDBusSettingsService::signal_delete_usr_word, this,
            [this](int flg, const QString &wordText, const QString &wordCode) {
                if (flg == 0)
                {
    ...
    connect(s, &QDBusSettingsService::signal_usr_word_load_ok, this, []() { settings::instance().set_userWordFlg(0); });
    connect(s, &QDBusSettingsService::signal_quick_table_load_ok, this, []() { settings::instance().set_quickTableFlg(0); });
    connect(s, &QDBusSettingsService::signal_ime_table_load_ok, this, []() { settings::instance().set_imeTableChanged(0); });

    connect(s, &QDBusSettingsService::signal_panel_exit, this, [this]() {
        qDebug() << "Freewb quit!";
        m_toolbar->hide();
        m_inputWin->hide();
    });

    connect(s, &QDBusSettingsService::signal_switch_vk, m_virtualKeyboard, &Keyboard::switch_vk);
    connect(s, &QDBusSettingsService::signal_close_vk, m_virtualKeyboard, &Keyboard::slot_toggle_win);
    connect(s, &QDBusSettingsService::signal_switch_char_set, m_toolbar, &ToolbarWin::switch_char_set);
    connect(s, &QDBusSettingsService::signal_switch_simp_or_trad, m_toolbar, [this]() {
        m_toolbar->set_traditional_mode(!ToolbarWin::is_traditional_mode());
    });
    connect(s, &QDBusSettingsService::signal_switch_caps_state, m_toolbar, &ToolbarWin::slot_update_input_mode_ico);

    connect(s, &QDBusSettingsService::signal_switch_toolbar_hide_flg, this, [this]() {
    ...
    connect(s, &QDBusSettingsService::signal_switch_candiwin_hide_flg, this, []() {
    ...
    connect(s, &QDBusSettingsService::signal_switch_lexicon, this, [this]() {
    ...
    connect(s, &QDBusSettingsService::signal_switch_skin, this, []() {
    ...
    connect(s, &QDBusSettingsService::signal_set_mark_auto_pairs_flg, this,
            [](int flg) { settings::instance().set_smartMark(flg != 0); });
    connect(s, &QDBusSettingsService::signal_set_recode_calib_flg, this,
            [](int flg) { settings::instance().set_recodeCalib(flg != 0); });

    connect(s, &QDBusSettingsService::signal_set_charWidth_and_markMode, this, [this](int charWidth, int markMode) {
        if (charWidth)
        {
    ...
    connect(s, &QDBusSettingsService::signal_open_freewb_dir, this, []() {
        char cmd[128] = {0};
        sprintf(cmd, "xdg-open %s > /dev/null 2>&1 &", QString(INSTALL_DIR).toUtf8().data());
        system(cmd);
    });

    connect(s, &QDBusSettingsService::signal_word_freq_switch_ok, this, [](const QString &wordText, int flg) {
    ...
    connect(s, &QDBusSettingsService::signal_open_ui_setting, m_settingWin, &SettingWin::slot_open_win);
    connect(s, &QDBusSettingsService::signal_open_advanced_setting, this,
            [this]() { m_textEditWin->slot_open_textEdit_win(TEM_SETTING_FILE); });
    connect(s, &QDBusSettingsService::signal_edit_usr_table, this,
            [this]() { m_textEditWin->slot_open_textEdit_win(TEM_USER_WORD); });
    connect(s, &QDBusSettingsService::signal_edit_wubi_table, this,
            [this]() { m_textEditWin->slot_open_textEdit_win(TEM_WUBI_TABLE); });
    connect(s, &QDBusSettingsService::signal_edit_pinyin_table, this,
            [this]() { m_textEditWin->slot_open_textEdit_win(TEM_PINYIN_TABLE); });
    connect(s, &QDBusSettingsService::signal_edit_quick_table, this,
            [this]() { m_textEditWin->slot_open_textEdit_win(TEM_QUICK_TABLE); });
    connect(s, &QDBusSettingsService::signal_show_version_info, m_settingWin, &SettingWin::slot_show_version_info);
}
