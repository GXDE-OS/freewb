...
#define FREEWB_CFG_Int_APPLY(id, GetFn, SetFn)                                                                           \
    int GetFn() const { return readInt(#id); }                                                                           \
    void SetFn(int v) { writeInt(#id, v); }

#define FREEWB_CFG_Bool_APPLY(id, GetFn, SetFn)                                                                          \
    bool GetFn() const { return readBool(#id); }                                                                         \
    void SetFn(bool v) { writeBool(#id, v); }

#define FREEWB_CFG_String_APPLY(id, GetFn, SetFn)                                                                        \
    const std::string &GetFn() const { return readString(#id); }                                                         \
    void SetFn(const std::string &v) { writeString(#id, v); }
...
#define FREEWB_AUTO_ACCESSOR_ENTRY_LIST(X)                                                                               \
    ...
    X(recodeSelectKey, CandidateWinOptions, Int, "1", CandidateWinOptions, "")                                           \
...
private:
    ...
    void appendEntriesFromDef()
    {
        entries_.clear();
        entries_.reserve(96);
#define FREEWB_CONFIG(id, cat, vtype, def, sect, desc)                                                                 \
    entries_.emplace(#id,                                                                                                \
                     ConfigEntry{#id, ValueType::vtype, makeValueSlots(ValueType::vtype, def), #sect, makeValueSlots(ValueType::vtype, def)});
