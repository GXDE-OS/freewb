...
static void SetIMIcon(FcitxInstance *instance, char **prop)
{
    ...
    else if (FcitxInstanceGetCurrentStatev2(instance) == IS_ACTIVE)
    {
        ...
        if (im)
        {
            if (strncmp(im->uniqueName, "fcitx-keyboard-", strlen("fcitx-keyboard-")) == 0)
            {
                if (isUnity())
                {
                    layout[INDICATOR_KEYBOARD_LENGTH] = toupper(im->langCode[0]);
    ...
    fcitx_utils_alloc_cat_str(*prop, "/Fcitx/im:", imname, icon_prefix, icon, ":", description, ":menu");
}
...
void FreeWubiPanelProxyInitializeInstance(FcitxInstance *instance)
{
    ...
    int id = FcitxDBusWatchName(instance, FREEWUBI_PANEL_SERVICENAME, freeWubiPanel, FreeWubiPanelProxyOwnerChanged, NULL, NULL);
    if (id == 0)
    {
        return;
    }
    ...
    dbus_bool_t reply = dbus_connection_send_with_reply(freeWubiPanel->conn, message, &call, DBUS_TIMEOUT_USE_DEFAULT);
    if (reply == TRUE)
    {
        dbus_pending_call_set_notify(call, FreeWubiPanelProxyServiceExistCallback, freeWubiPanel, NULL);
...
int FreeWubiUINewMessageToOldStyleMessage(FcitxInputState *input, FcitxMessages *msgUp, FcitxMessages *msgDown)
{
         ...
         candWord = FcitxCandidateWordGetCurrentWindowNext(input->candList, candWord), i++)
    {
        ...
        if (i == 0 && FcitxCandidateWordGetCurrentPage(input->candList) == 0 && type == MSG_OTHER
        ...
        if (candWord->strExtra && strlen(candWord->strExtra))
            FcitxMessagesAddMessageStringsAtLast(msgDown, candWord->extraType, candWord->strExtra);
...
static DBusHandlerResult FreeWubiPanelProxyDBusEventHandler(DBusConnection *connection, DBusMessage *msg, void *arg)
{
...
{
    return dbus_message_new_signal(FREEWUBI_INPUTMETHOD_OBJECTPATH, FREEWUBI_INPUTMETHOD_SERVICENAME, signalName);
}
